void up_down() {
  ...
  for (auto i=1; i<=nbins_u; i++) {
    ...
    auto pt = new TPaveText(tmax -0.6, 0.4, tmax + 0.2, 0.7, "nb");
    //auto pt = new TPaveText(tmax -2, 0.5, tmax + 2, 0.75, "nb");
    pt->AddText(s_entries);
    pt->AddText("Tempo di acquisizione: 29 h");
